...
function initMap() {
	...
	google.maps.event.addListener(map, 'idle', function(){
	    ...
	    if(markerLength == 0){	    	
	    	switch($('.favorite-menu ul li button.active').html()){
		    	case "최근 본 방":
		    		$('.visited').removeClass('hidden');
		    		break;
		    	case "찜한 방":
		    		$('.ListZero.favorited').removeClass('hidden');
		    		break;
		    	case "연락한 방":
...
$(function(){
	$('.favorite-menu ul li button').click(function(){	
		switch($(this).html()){
			case "최근 본 방":
				favorite("/CBangProj/Favorite/Visited.do");
				break;
			...
			case "연락한 방":
				...
				favorite("/CBangProj/Favorite/Contacted.do");
				break;					
		}
		
		$('.favorite-menu ul li button').removeClass('active');
		$(this).addClass('active');		
		$('.new-list-rooms-title').html($(this).html());				
	});
});